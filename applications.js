	...
	if (editor.includes(message.author.id) == true) {
		...
		for (let i = 2; i < 250; i++) {
			...
			try {
				...
				applicant = client.users.cache.get(tag.value) //client.users.cache.find(member => member.user.id == tag.value) //guild.members.cache.find(member => member.user.tag == tag.value)
			} catch(err) {
				console.error(err)
			}
			if (mark.value === null) {
